...
Node *find_repost(Node *post, int repost_id) {
  if (post->id == repost_id) return post;
...
void create_repost(char *cmd, Node **posts, int nr_posts, int *id_curent) {
  ...
  if (s3 == NULL) {  // repost la post
    ...
    aux->events[aux->events_number]->persons_like =
        (int *)calloc(MAX_PEOPLE, sizeof(int));
    aux->events_number++;
...
void dfs_print(Node *post, int contor) {
  ...
  for (int i = 0; i < post->events_number; i++) {
    if (post->events[i]) dfs_print(post->events[i], 0);
  }
}
...
void dfs_ratio_max_likes(Node *post, int *max, int *indice) {
  if (post->number_of_likes > *max) {
    *max = post->number_of_likes;
    if (*indice < post->id) *indice = post->id;
  }
  for (int i = 0; i < post->events_number; i++) {
    if (post->events[i]) dfs_ratio_max_likes(post->events[i], max, indice);
  }
}
...
void delete_post(Node *posts) {
  ...
  if (posts->titlu) free(posts->titlu);
...
void delete(char *cmd, Node **posts, int *nr_posts) {
  ...
  if (s2 == NULL) {
    printf("Deleted %s\n", aux->titlu);
    delete_post(posts[i]);
    for (int j = i; j < *nr_posts - 1; j++) posts[j] = posts[j + 1];
...
void dfs_parinti(Node *node, int ant, int **parents) {
  ...
  for (int i = 0; i < node->events_number; i++) {
    if (node->events[i]) dfs_parinti(node->events[i], node->id, parents);
  }
}
...
void common_repost(char *cmd, Node **posts, int nr_posts) {
  ...
  if (i == -1 && j == -1) {
    printf("The first common repost of %d and %d is %d\n", repost_id1,
           repost_id2, parinti2[0]);
  } else if (i == -1) {
    if (parinti2[j] == repost_id1)
      printf("The first common repost of %d and %d is %d\n", repost_id1,
             repost_id2, parinti2[j]);
    else
      printf("The first common repost of %d and %d is %d\n", repost_id1,
             repost_id2, parinti2[j + 1]);
  } else if (j == -1)
    if (parinti1[i] == repost_id2)
      printf("The first common repost of %d and %d is %d\n", repost_id1,
             repost_id2, parinti1[i]);
    else
      printf("The first common repost of %d and %d is %d\n", repost_id1,
             repost_id2, parinti1[i + 1]);
...
void handle_input_posts(char *input, Node **posts, int *nr_posts,
                        int *id_curent) {
